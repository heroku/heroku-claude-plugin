...
## Step 3 — Check .heroku-plugin-scaffold.json

`app.json` is not generated by this plugin — do not flag its absence.
Instead verify `.heroku-plugin-scaffold.json` exists and contains:
- `addons` — list of addon slugs the app expects
- `secret_env_vars` — list of env var names requiring generated secrets
...
- .heroku-plugin-scaffold.json: present, addons and secret_env_vars declared
- ...
